...
class EditExamSheet extends StatefulWidget {
  ...
  @override
  State<EditExamSheet> createState() => _EditExamSheetState();
}

class _EditExamSheetState extends State<EditExamSheet> {
  ...
  void initState() {
    ...
    _nameCtrl.text = exam.name;
    _totalMarksCtrl.text = exam.maxScore
        .toString(); // keep using maxScore from model
    _durationCtrl.text = exam.durationMinutes.toString();
    _selectedSubjectId = exam.subjectId;
    _selectedClassroomId = exam.classroomId;
    _selectedDate = exam.scheduledAt != null
        ? DateTime.tryParse(exam.scheduledAt!)
        : null;
  }
  ...
  void _submit() async {
    ...
    // ✅ Correct payload keys (same as create)
    final data = {
    ...
    try {
      await widget.cubit.updateExam(widget.exam.id, data);
